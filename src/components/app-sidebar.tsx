...
const getNavigationItems = (userRole: string | undefined, hasPermission: (permission: keyof RolePermissions) => boolean) => {
  ...
  // Subconsultant Dashboard (only for subconsultants)
  if (hasPermission('canAccessSubconsultantDashboard')) {
    navMain.push({
      title: "Dashboard",
      url: "/subconsultant-dashboard",
      icon: LayoutDashboard,
    });
  }
      ...
      <SidebarHeader>
          <SidebarMenu>
              <SidebarMenuItem>
                  ...
                  >
                      <Link href={user?.role === 'Subconsultant' ? '/subconsultant-dashboard' : '/dashboard'}>
                            <Image
