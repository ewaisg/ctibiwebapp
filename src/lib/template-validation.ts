...
import { validateTemplateFieldName, sanitizeTemplateValue, sanitizePdfFieldName } from './security-utils';
...
export function sanitizeFieldMapping(mapping: any): TemplateFieldMapping {
  ...
  // Auto-sanitize field names to be database-safe
  const originalFieldName = sanitizeTemplateValue(mapping.fieldName) || '';
  const sanitizedFieldName = sanitizePdfFieldName(originalFieldName);

  const sanitized: any = {
    fieldName: sanitizedFieldName || originalFieldName,
