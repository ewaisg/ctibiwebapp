...
interface ProjectHoursStackedProps {
  data?: ProjectHoursDatum[];
  title?: string;
  subtitle?: string;
}

function ProjectHoursStacked({ data = [] as ProjectHoursDatum[], title = "Project Hours", subtitle = "Budget vs actual" }: ProjectHoursStackedProps) {
      ...
      {!hasData ? (
        ...
        <ChartContainer config={config} className="h-64">
          <BarChart data={data} accessibilityLayer margin={{ top: 8, right: 16, left: 4, bottom: 8 }} barSize={18} barCategoryGap={12}>
            ...
            <Bar dataKey="budgetedHours" fill="var(--chart-3)" stackId="a" radius={[0, 0, 0, 0]} />
            <Bar dataKey="actualHours" fill="var(--chart-1)" stackId="a" radius={[0, 0, 0, 0]} />
...
export default memo(ProjectHoursStacked);
