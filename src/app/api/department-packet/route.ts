...
export async function POST(request: NextRequest) {
  const authed = await withAuth(async (req) => {
    try {
      ...
      const coverPageBuffer = await generateCoverPage({ departmentId: sanitizedDepartmentId, fromDate, toDate, manualData: manualData || {} });

      // 2) Department Summary data build
      // Department
      const depDoc = await adminDb!.collection('departments').doc(sanitizedDepartmentId).get();
      ...
      const depRef = adminDb!.doc(`departments/${sanitizedDepartmentId}`);
      const [projByRefSnap, projByIdSnap] = await Promise.all([
        adminDb!.collection('projects').where('departmentId', '==', depRef).get(),
        adminDb!.collection('projects').where('departmentId', '==', sanitizedDepartmentId).get(),
      ]);
