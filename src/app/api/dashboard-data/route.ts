...
import { sanitizeDocumentId, validateTimeRange, sanitizeEmployeeId } from '@/lib/security-utils';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Sanitize and validate all query parameters to prevent NoSQL injection
    const timeRangeRaw = searchParams.get('timeRange');
    const projectIdRaw = searchParams.get('projectId');
    const employeeIdRaw = searchParams.get('employeeId');

    const timeRange = validateTimeRange(timeRangeRaw) || undefined;
    const projectId = sanitizeDocumentId(projectIdRaw) || undefined;
    const employeeId = sanitizeEmployeeId(employeeIdRaw) || undefined;

    // Use authenticated user from withAuth middleware
  ...
  return withRateLimit(handler, { maxRequests: 30, windowMs: 60 * 1000 })(request);
}