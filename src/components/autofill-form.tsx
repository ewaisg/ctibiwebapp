...
export function AutofillForm({ projects, departments = [], onAutofillComplete, onCancel }: AutofillFormProps) {
  ...
  const checkTimesheetData = useCallback(async () => {
    try {
      ...
      const { autofillFromTimesheets } = await import('@/app/(authenticated)/invoicing/invoice-autofill-actions');
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
          ...
          {isProcessing && (
            <div className="rounded-lg border p-4 flex items-center gap-3">
              ...
              <span className="text-sm">Checking timesheet entries...</span>
            </div>
          )}
          ...
          {timesheetPreview && !isProcessing && (
            <div className={cn(
              ...
              timesheetPreview.hasData ? "border-green-500/50 bg-green-50/20" : "border-yellow-500/50 bg-yellow-50/20"
            )}>
              ...
              {timesheetPreview.hasData ? (
                <div className="grid grid-cols-2 gap-2 text-sm pl-4">
                  ...
                  Total Hours: {timesheetPreview.totalHours}
                  </div>
                </div>
              ) : (
