      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const uniqueKey = `${String((item as any)?.dataKey ?? (item as any)?.name ?? 'value')}-${String((item as any)?.color ?? '')}-${index}`
                    ...
                    >
                      ...
                      {item.value && (
                        <span className="font-mono font-medium tabular-nums text-foreground">
                          {typeof item.value === 'number' ? item.value.toLocaleString() : String(item.value || '').replace(/[<>"'&]/g, '')}
                        </span>
                      )}
                    </div>
                  </>
    ...
    return (
      ...
      >
        {payload.map((item, index) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
          const uniqueKey = `${String((item as any)?.dataKey ?? (item as any)?.value ?? index)}-${String((item as any)?.color ?? "")}`
