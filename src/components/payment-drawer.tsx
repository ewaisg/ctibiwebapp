...
export function PaymentDrawer({ open, onOpenChange, invoice }: PaymentDrawerProps) {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        const { getPaymentHistory } = await import("@/app/(authenticated)/admin/admin-payment-actions");
  ...
  const applyEdit = async () => {
    ...
    setSavingEdit(true);
    try {
      const { updatePaymentEntry, getPaymentHistory } = await import("@/app/(authenticated)/admin/admin-payment-actions");
  ...
  const voidEntry = async (entry: any) => {
    ...
    try {
      const { voidPaymentEntry, getPaymentHistory } = await import("@/app/(authenticated)/admin/admin-payment-actions");
  ...
  const handleSaveEntry = async () => {
    ...
    setSavingEntry(true);
    try {
      const { addPaymentEntry, getPaymentHistory } = await import("@/app/(authenticated)/admin/admin-payment-actions");
      const payload = {
        invoiceId: invoice.id as string,
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="flex justify-end gap-2">
                <Button variant="destructive" onClick={async () => {
                  try {
                    const mod = await import("@/app/(authenticated)/admin/admin-payment-actions");
