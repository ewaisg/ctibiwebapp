...
async function fillPdfTemplate(
  ...
  // Try new template system first, fallback to legacy
  ...
  try {
    ...
    if (template) {
      ...
      const [contracts, departments, companiesData] = await Promise.all([
        getCollectionData('contracts'),
      ...
      // Fallback to legacy system for invoices only
      pdfTemplateBytes = await getInvoicePdfTemplate((invoiceData as any).contractNumber);
    }
  } catch (error) {
    console.warn('Template resolver failed, using legacy system:', error);
    pdfTemplateBytes = await getInvoicePdfTemplate((invoiceData as any).contractNumber);
  }
