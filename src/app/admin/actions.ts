...
export async function recordPayment(paymentData: {
  ...
  try {
    ...
    // Exclude timestamp fields from client response
    const { paymentDate, dueDate, createdAt, updatedAt, ...clientSafePayment } = paymentDoc;
    
    return {
      success: true,
      message: "Payment recorded successfully",
      payment: { id: docRef.id, ...clientSafePayment }
    };
  } catch (error) {
...
async function generateRevenueReport(dateRange: { startDate: Date; endDate: Date }, filters: any) {
...
async function generateOutstandingReport(dateRange: { startDate: Date; endDate: Date }, filters: any) {
  // Implementation for outstanding report generation
  return {
    totalOutstanding: 0,
...
async function generateUtilizationReport(dateRange: { startDate: Date; endDate: Date }, filters: any) {
  // Implementation for utilization report generation
  return {
...
async function generateProfitabilityReport(dateRange: { startDate: Date; endDate: Date }, filters: any) {
  // Implementation for profitability report generation
  return {
    grossProfit: 0,
    profitMargin: 0,
    profitByProject: [],
