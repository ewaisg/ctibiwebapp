...
export async function generateDepartmentalCompilation(
  ...
  try {
    ...
    for (const invoice of relevantInvoices) {
      ...
      const attachments = invoice.id ? await getInvoiceAttachments(invoice.id) : [];      // Generate Direct Labor Report from autofilled timesheet data
      ...
      try {
        ...
        if (shouldGenerateLaborReport(invoice)) {
          ...
          console.log('[Labor Report] Generated bytes for invoice:', sanitizeForLog(timecardReport?.length || 0));
        } else {
          console.log('[Labor Report] Skipping invoice - no timesheet data');
        }
      } catch (error) {
        console.warn('Failed to generate Direct Labor Report for invoice:', sanitizeForLog(error));
      }
    ...
    for (const [projectId, submissions] of submissionsByProject) {
      ...
      for (const [submissionDate, invoiceSubmissions] of submissions) {
        ...
        for (const submission of invoiceSubmissions) {
          ...
          // Add Direct Labor Report if available
          ...
          // Add existing Direct Labor Report if available
          if (submission.invoice.directLaborReportUrl && submission.invoice.directLaborReportFileName) {
            try {
              ...
              console.warn('Failed to download direct labor report for invoice:', sanitizeForLog(error));
            }
          }
